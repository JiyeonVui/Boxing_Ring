...
public class AudioManager : MonoSingleton<AudioManager>
{
    ...
    public void PlayMusic(AudioType audioType, float volume = 1f, bool isLoop = true)
    {
        if (!isInit)
            return;

        bool isSoundOn = DataManager.Instance.GetData<DataUser>().HasSound();
        if (!isSoundOn)
        {
            return;
        }
    ...
    public void PlaySoundRandom(AudioType audioType)
    {
        AudioClip audioClip = audioAsset.GetRandomClip(audioType);
